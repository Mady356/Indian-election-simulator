...
Run as:
    python -m src.clean.clean_eci

Year-agnostic: the script reads from `RAW_YEAR_DIR` (driven by ACTIVE_YEAR
in `src.config`) and writes to filenames derived from the same constant.
Flipping ACTIVE_YEAR in config.py is the only edit needed to clean a
different year — there are no per-year copies of this file.

This loader targets the modern ECI .xls / .xlsx file format used for 2019
and 2024. If a future year ships in a substantially different layout, add a
sibling loader (e.g. clean_eci_legacy.py) rather than overloading this one.
...
# Make the script runnable both as a module (`python -m src.clean.clean_eci`)
# and as a plain script (`python src/clean/clean_eci.py`).
sys.path.insert(0, str(Path(__file__).resolve().parents[2]))

from src.config import ACTIVE_YEAR, RAW_YEAR_DIR
...
# When you load a new year and hit a "missing column" error, add the new variant
# to whichever canonical name it should land on.
RESULTS_RENAME_MAP = {
    # state / constituency
    "state": "state",
    "state_name": "state",
    ...
    # candidate (2019 ECI uses the plural "candidates_name")
    "candidate": "candidate",
    "candidate_name": "candidate",
    "candidates_name": "candidate",
    # party
    "party": "party",
    "party_name": "party",
    # vote breakdown
    "evm_votes": "evm_votes",
    "general": "evm_votes",          # 2019 ECI header
    "postal_votes": "postal_votes",
    "postal": "postal_votes",        # 2019 ECI header
    # totals
    "total_votes": "votes",
    "total": "votes",
    "votes": "votes",
