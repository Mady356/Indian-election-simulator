# Research packet: Kokrajhar, Assam

_Generated 2026-06-22. Evidence-backed prompts only; no invented local facts._

## Identity
- State: Assam (`ASSAM`)
- Constituency: Kokrajhar (`KOKRAJHAR`)

## Election facts (2019 → 2024)
- Winner 2019: Naba Kumar Sarania
- Winner 2024: JOYANTA BASUMATARY
- Party 2019: IND
- Party 2024: UPPL
- Winner changed: True
- BJP vote share 2019: None
- BJP vote share 2024: None
- INC vote share 2019: 9.9439
- INC vote share 2024: 9.1612
- BJP swing: None
- INC swing: -0.7827
- Margin 2019: 2.554
- Margin 2024: 4.1549
- Margin change: 1.6009
- Turnout 2019: 27.2783
- Turnout 2024: 32.72
- Turnout change: 5.4417

## State context
- State seats: 14
- BJP seats 2019/2024: 9 / 9
- INC seats 2019/2024: 3 / 3
- State avg BJP swing: 1.62
- State avg INC swing: 0.37
- State demographic coverage: 57.14%

## Rank / context flags
- is_flipped_seat: True
- is_close_2024: True
- is_large_bjp_gain: False
- is_large_bjp_loss: False
- is_large_inc_gain: False
- is_large_inc_loss: False
- is_priority_seat: True
- priority_reason: seat_flip
- priority_rank: 210

## Demographic context
- data_quality_label: high
- electricity_pct_nfhs5: 15.0867
- improved_sanitation_pct_nfhs5: 65.705
- lpg_pct_nfhs5: 6.125
- mobile_phone_pct_nfhs5: 2.016
- urban_pct_nfhs5: 19.2734
- nfhs5_coverage_share: 1.0
- change_coverage_share: 1.0
- change_quality_flag: medium
- districts_used: GOALPARA→Goalpara (NFHS-4 proxy); KAMRUP→Kamrup Metropolitan (NFHS-4 proxy)
- districts_missing: N/A

## Simulation context
- simulation_completeness: full_party_shares
- winner_2024: JOYANTA BASUMATARY
- winner_party_2024: UPPL
- data_quality_label: high
- projection_confidence_hint: high

## Existing analysis
- analysis_source: generated
- generated summary: Kokrajhar flipped from IND to UPPL in 2024. INC vote share changed by -0.8 points compared with 2019. The 2024 margin was 4.2%. This is a descriptive election profile, not a causal explanation.
- generated movement: Available data indicates Kokrajhar flipped from IND to UPPL between 2019 and 2024. INC vote share changed by -0.8 percentage points versus 2019. The winning margin moved from 2.6% in 2019 to 4.2% in 2024. Turnout changed by +5.4 percentage points between 2019 and 2024. This section describes observed election movement only and is not a causal explanation.
- has manual markdown: False

## Writing prompts

- **what_happened_prompt**: Describe observed 2019→2024 election movement for Kokrajhar, Assam using winner, vote-share, margin, and turnout fields only.
- **why_it_mattered_prompt**: Explain why Kokrajhar may be analytically useful using priority reason (seat_flip), contest closeness, swings, and state context.
- **factors_prompt**: List only cautious factors such as candidate profile, alliance arithmetic, state-level swing, turnout movement, or limited demographic coverage.
- **demographic_prompt**: Summarise available NFHS-linked indicators and district mapping. If unavailable, state that the profile remains election-only.
- **what_to_watch_prompt**: Use forward-looking but non-predictive language about seat stability, swing consolidation, and coverage improvements.
- **caveat_prompt**: State that local causal explanations require further source review.
